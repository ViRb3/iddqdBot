...
public class AStarPathFinder implements PathFinder {
	...
	private org.newdawn.slick.util.pathfinding.TileBasedMap map;
	...
	public Path findPath(Mover mover, int sx, int sy, int tx, int ty) {
		...
		//TODO: Maybe not a good idea to remove?
		...
		while ((maxDepth < maxSearchDistance) && (open.size() != 0)) {
			...
			for (int x=-1;x<2;x++) {
				for (int y=-1;y<2;y++) {
					...
					if (isValidLocation(mover,sx,sy, current.x, current.y, xp,yp)) {
	 ...
	 */
	protected boolean isValidLocation(Mover mover, int sx, int sy, int cx, int cy, int x, int y) {
		boolean invalid = (x < 0) || (y < 0) || (x >= map.getWidthInTiles()) || (y >= map.getHeightInTiles());
		
		if ((!invalid) && ((sx != x) || (sy != y))) {
			invalid = map.blocked(mover, cx, cy, x, y);
		}
